...
const WrapperVideo = ({ children }) => {
    ...
    const handleOpenCallVideo = () => {
        ...
        const newWindow = window.open(`/video-call?chat=${chat._id}&isCalled=false`, '_blank', `width=${width},height=${height},left=${left},top=${top}`);
        if (newWindow) {
    ...
    return (
        <span onClick={handleOpenCallVideo}>{children}</span>
    );
}
